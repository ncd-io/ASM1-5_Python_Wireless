...
while True:
    command = input('Enter Channel Number: ')
    if command == 'exit':
    ...
    command = input('Enter Desired Voltage: ')
    if command == 'exit':
        if device is not None and device.is_open():
            device.close()
        exit(1)
    desiredVoltage = float(command)

    rawDAC = int(desiredVoltage/(10.00/4095))
    print('rawDAC: ',rawDAC)
    lsb = rawDAC&255
    msb = (rawDAC>>8)&255
    checksum = (0xF3+channel+channel+msb+lsb)&255
    commandBytes = bytearray([0xAA, 0x06, 0x35, 0x0E, channel, channel, msb, lsb, checksum])

    try:
        device.send_data(remote_device, commandBytes, 6)
        print('Channel: ',channel,' set to voltage: ',desiredVoltage)
    except:
